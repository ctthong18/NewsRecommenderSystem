...
class UserEncoder(nn.Module):
    ...
    def forward(self, news_histories: torch.Tensor, news_encoder: nn.Module, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if attention_mask is not None:
            attention_mask_flat = attention_mask.view(batch_size * hist_size, seq_len)

        news_histories_encoded = news_encoder(
            news_histories, attention_mask=attention_mask_flat
